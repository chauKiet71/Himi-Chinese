...
export default async function AdminPage({ searchParams }: {
  ...
  return <main className="admin-page"><div className="section-shell">
    ...
    <div className="admin-grid">
      <section className="admin-panel"><div className="panel-heading"><div><span>Danh mục nội dung</span><h2>Quản lý lộ trình</h2></div><AdminLink href="/admin/courses">Mở CRUD nội dung</AdminLink></div><div className="table-scroll"><table className="data-table"><thead><tr><th>Lộ trình</th><th>Bài học</th><th>Trạng thái</th><th>Cập nhật</th></tr></thead><tbody>{data.courses.map((course) => <tr key={course.id}><td><AdminLink className="table-course" href={`/admin/courses/${course.id}`} intentPrefetch><span className="table-mark">{course.hanzi}</span><span><strong>{course.titleVi}</strong><small>{course.slug}</small></span></AdminLink></td><td>{course.lessonCount}</td><td><StatusBadge status={course.status} /></td><td>{course.updatedAt.toLocaleDateString("vi-VN")}</td></tr>)}</tbody></table></div></section>
