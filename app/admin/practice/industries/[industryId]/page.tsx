...
export default async function AdminPracticeIndustryPage({ params, searchParams }: {
  ...
  return <main className="admin-page"><div className="section-shell">
    ...
    <div className="admin-content-grid modules-section">
      ...
      <section className="admin-panel"><div className="panel-heading"><h2>{data.scenarios.length} ca</h2><span>Theo thứ tự hiển thị</span></div><div className="admin-record-list practice-admin-list">{data.scenarios.length ? data.scenarios.map((scenario) => <AdminLink href={`/admin/practice/scenarios/${scenario.id}`} intentPrefetch key={scenario.id}><span><strong>{scenario.title}</strong><small>{scenario.slug} · {scenario.durationMinutes} phút · {scenario.exerciseCount} lượt nghe · {scenario.attemptCount} lượt làm · {scenario.isFree ? "Miễn phí" : "VIP"}</small>{scenario.status === "review" ? <small className="practice-review-list-note">{scenario.reviewerId ? `Reviewer: ${scenario.reviewerName || scenario.reviewerEmail}` : "Chưa có reviewer"}{scenario.reviewDueAt ? ` · Hạn ${scenario.reviewDueAt.toLocaleDateString("vi-VN", { timeZone: "Asia/Ho_Chi_Minh" })}` : ""}</small> : null}</span><StatusBadge status={scenario.status} /></AdminLink>) : <p className="admin-empty">Chưa có ca nào trong nhóm này.</p>}</div></section>
    </div>
  </div></main>;
}
