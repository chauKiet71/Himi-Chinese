...
import { isDatabaseUnavailableError, withRequestDb } from "@/db/index";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    console.warn("[auth] database unavailable; login temporarily disabled");
    return NextResponse.redirect(authRedirectUrl(request, loginPath, { error: "service_unavailable", returnTo }), 303);
  }
}
