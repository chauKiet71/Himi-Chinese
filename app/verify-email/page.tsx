...
export default async function VerifyEmailPage({ searchParams }: { searchParams: Promise<{ token?: string; sent?: string; required?: string; email_changed?: string; error?: string }> }) {
  const params = await searchParams;
  const token = params.token?.slice(0, 128) ?? "";
  const hasToken = validateAuthToken(token);
  const cookieStore = await cookies();
  const pendingEmail = normalizeEmail(cookieStore.get(pendingEmailVerificationCookieName())?.value ?? "");
  ...
  return <main className="auth-page"><section className="auth-card">
    ...
    {hasToken ? <form action="/api/auth/verify-email" className="auth-form" method="post">
      ...
      <form action="/api/auth/resend-verification" className="auth-resend-form" method="post">
        ...
        <button className="button button-secondary button-full" type="submit">Gửi lại mã</button>
      </form>
      ...
      <button className="button button-secondary button-full" type="submit">Gửi mã xác minh</button>
    </form>}
