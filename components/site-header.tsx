...
export async function SiteHeader() {
  ...
  return <header className="site-header"><div className="section-shell header-inner">
    ...
    <div className="header-actions">
      {user ? <><Link className="header-login" href="/account" prefetch={false}><UserRound size={17} /> {user.displayName}</Link><LogoutForm className="header-logout"><button aria-label="Đăng xuất" title="Đăng xuất" type="submit"><LogOut size={17} /></button></LogoutForm></> : <Link className="header-login" href="/login" prefetch={false}><UserRound size={17} /> Đăng nhập</Link>}<Link className="button button-primary button-small" href="/courses" prefetch={false}>Học miễn phí</Link>
