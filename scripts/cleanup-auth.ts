...
import { adminLoginChallenges, authRateLimits, authSessions, authTokens } from "../db/schema.ts";
...
async function cleanupAuth() {
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const challenges = await tx.delete(adminLoginChallenges).where(or(
      lt(adminLoginChallenges.expiresAt, now),
      and(isNotNull(adminLoginChallenges.usedAt), lt(adminLoginChallenges.usedAt, staleBefore)),
    ...
    return { challenges: challenges.length, sessions: sessions.length, tokens: tokens.length, rateLimits: rateLimits.length };
  });
  console.log(`Đã dọn auth: ${result.sessions} session, ${result.challenges} challenge, ${result.tokens} token, ${result.rateLimits} rate-limit.`);
}
