"""Generate clean HSK 6 volume 1 article audio with Eleven v3 and Cloudinary."""
...
BUNDLE = ROOT / "content/hsk6-textbook-1-json"
CACHE = ROOT / ".codex-tmp/hsk6-v1-elevenlabs"
VOICE_ID = "9lHjugDhwqoxA5MhX0az"
MODEL_ID = "eleven_v3"
SOURCE_NOISE = re.compile(r"HSK\s*标准教程|标准教程6|www\.|nhantriviet|GiaoTrinhChuanHSK", re.I)
...
def main() -> None:
    ...
    for article in texts["texts"]:
        ...
        attributions = list(re.finditer(r"改编自|摘自|选自", source_text))
        text = (source_text[:attributions[-1].start()]
                if attributions and attributions[-1].start() > len(source_text) * 0.7
                else source_text)
        if (not text or OCR_ARTIFACT.search(text)
        ...
        with mp3_path.open("rb") as audio:
            uploaded = requests.post(
                ...
                data={"public_id": f"hanziwork/hsk6/volume1-articles/{asset['id']}", "overwrite": "true"},
    ...
    save_json(texts_path, texts)
    print(f"HSK 6 volume 1: generated {completed}; skipped OCR-contaminated {skipped}", flush=True)
