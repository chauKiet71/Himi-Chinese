...
test("HSK 5 workbook 1 converts all lessons into the HSK 1 learning shape", async () => {
  ...
  assert.equal(lessons.reduce((total, lesson) => total + lesson.dialogues.length, 0), 0);
  ...
  assert.ok(lessons.every((lesson) => lesson.dialogues.length === 0));
  ...
  assert.match(lesson.summary, /32 câu Nghe – Đọc – Viết/u);
  assert.match(lesson.summary, /chưa chấm điểm/u);
