...
test("slice game flies to the target, splits the word and reveals its Vietnamese meaning", async () => {
  ...
  assert.match(game, /CONSECUTIVE_MISSES_TO_END = 3/);
  assert.match(game, /setConsecutiveMisses\(0\)/);
  assert.match(game, /nextConsecutiveMisses >= CONSECUTIVE_MISSES_TO_END/);
  assert.match(game, /completionReportedRef\.current = true;[\s\S]*?onComplete\?\.\(pending\.finalScore, level\);[\s\S]*?advanceWord\(\)/);
